...
export default function EditInspectionRoutineScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();
  const { getInspectionRoutineById, updateInspectionRoutine, isLoading } = useFarmData();

  const routine = getInspectionRoutineById(id ?? '');
  ...
  const removeChecklistItem = (itemId: string) => {
    if (checklistItems.length <= 1) {
      Alert.alert('Cannot Remove', 'An inspection routine must have at least one checklist item.');
      return;
    }
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (!name.trim()) {
        throw new Error('Inspection routine name is required');
      }
        ...
        >
          <View style={styles.section}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="e.g., Pre-Season Inspection"
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              >
                <Plus color={INSPECTION_COLOR} size={18} />
                <Text style={[styles.addItemText, { color: INSPECTION_COLOR }]}>Add Item</Text>
              </TouchableOpacity>
            </View>
